...
function InitializeWindow {             
	...
	If ($dsWindow.FindName("expTermSearch")) {			
		Try {
			Import-Module -FullyQualifiedName "C:\ProgramData\Autodesk\Vault 2027\Extensions\DataStandard\Vault.Custom\addinVault\ADSK.TS.FileTermExpander.psm1"
		}
		catch {}
	}
...
function AddinLoaded {
	...
	$m_File = "$($env:appdata)\Autodesk\DataStandard 2027\ScTemplate.xml"
	if (!(Test-Path $m_File)) {
		$source = "$($env:ProgramData)\Autodesk\Vault 2027\Extensions\DataStandard\Vault.Custom\ScTemplate.xml"
...
function mHelp ([Int] $mHContext) {
	try {
		...
		$mHelpTarget = $Env:ProgramData + "\Autodesk\Vault 2027\Extensions\DataStandard\HelpFiles\" + $mHPage
...
function mFillMyScTree {
	...
	$IconSource = "C:\ProgramData\Autodesk\Vault 2027\Extensions\DataStandard\Vault.Custom\Icons$($currentTheme)\User_CO_16.png"
	...
	$IconSource = "C:\ProgramData\Autodesk\Vault 2027\Extensions\DataStandard\Vault.Custom\Icons$($currentTheme)\User_Admin_16.png"
...
function mAddTreeNode($XmlNode, $TreeLevel, $EnableDelete) {
	...
	if ($XmlNode.LocalName -eq "ShortcutGroup") {
		$IconSource = "C:\ProgramData\Autodesk\Vault 2027\Extensions\DataStandard\Vault.Custom\Icons$($currentTheme)\FolderClosedMask_16.png"
...
function mGetIconSource {
	...
	[string]$ImagePath = "C:\ProgramData\Autodesk\Vault 2027\Extensions\DataStandard\Vault.Custom\Icons$($currentTheme)\Unknown_Sc_16x16.png"

	if ($ImageMetaData -like "*.iam?*") {
		return $ImagePath = "C:\ProgramData\Autodesk\Vault 2027\Extensions\DataStandard\Vault.Custom\Icons$($currentTheme)\IAM_Sc_16x16.png" 
	}
	if ($ImageMetaData -like '*.ipt?*') {
		return $ImagePath = "C:\ProgramData\Autodesk\Vault 2027\Extensions\DataStandard\Vault.Custom\Icons$($currentTheme)\IPT_Sc_16x16.png"
	}
	if ($ImageMetaData -like '*.ipn?*') {
		return $ImagePath = "C:\ProgramData\Autodesk\Vault 2027\Extensions\DataStandard\Vault.Custom\Icons$($currentTheme)\IPN_Sc_16x16.png"
	}
	if ($ImageMetaData -like "*.idw?*") {
		return $ImagePath = "C:\ProgramData\Autodesk\Vault 2027\Extensions\DataStandard\Vault.Custom\Icons$($currentTheme)\IDW_Sc_16x16.png"
	}
	if ($ImageMetaData -like '*.dwg?*') {
		return $ImagePath = "C:\ProgramData\Autodesk\Vault 2027\Extensions\DataStandard\Vault.Custom\Icons$($currentTheme)\DWG_Sc_16x16.png"
	}
	if ($ImageMetaData -like '*TAG=Folder*') {
		$FolderTemplate = "C:\ProgramData\Autodesk\Vault 2027\Extensions\DataStandard\Vault.Custom\Icons$($currentTheme)\FolderScToRecolor_16.png"
		...
		$FlrdArgbName = "$($env:appdata)\Autodesk\DataStandard 2027\FolderScColored-$($ARGBValues[0].Value)-$($ARGBValues[1].Value)-$($ARGBValues[2].Value)-$($ARGBValues[3].Value)_16.png"		
...
function mReadUserShortcuts {
	...
	$m_Path = "$($env:appdata)\Autodesk\VaultCommon\Servers\Services_Security_12_08_2025\$($m_Server)\Vaults\$($m_Vault)\Objects\"
...
function mAddShortCutByName([STRING] $mScName) {
	...
	try {
		...
		if (Test-Path $m_File) {
			#$dsDiag.Trace(">>-- Started to read ScTemplate.xml...")
