#[System.Reflection.Assembly]::LoadFrom("C:\Program Files (x86)\Autodesk\Autodesk Vault 2027 SDK\bin\Autodesk.Connectivity.WebServices.dll")
...
function InitializeNumSchm()
{
	...
	# Get numbering schemes with error handling
	try
	{
...
function GetTemplateFolders
{
	...
	$xmlpath = "$env:programdata\Autodesk\Vault 2027\Extensions\DataStandard\Vault\Configuration\File.xml"

	if ($_IsOfficeClient) {
		$xmlpath = "$env:programdata\Autodesk\Vault 2027\Extensions\DataStandard\Vault\Configuration\FileOffice.xml"
	}
